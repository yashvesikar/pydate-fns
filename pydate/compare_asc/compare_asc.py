...
def compare_asc(date_left: Union[datetime, float, int], date_right: Union[datetime, float, int]) -> float:
    """
    ...
    Example:
        ...
        >>> result = compare_asc(datetime(1987, 2, 11), datetime(1989, 7, 10))
        >>> # => -1
        ...
        >>>     datetime(1987, 2, 11),
        >>>     datetime(1989, 7, 10)
        >>> ], key=lambda x: compare_asc(x, datetime.now()))
        >>> # => [
        >>> #   datetime(1987, 2, 11),
        >>> #   datetime(1989, 7, 10),
        >>> #   datetime(1995, 7, 2)
    ...
    """
    # Handle NaN inputs
    if isinstance(date_left, float) and date_left != date_left:
        return float('nan')
    if isinstance(date_right, float) and date_right != date_right:
        return float('nan')
