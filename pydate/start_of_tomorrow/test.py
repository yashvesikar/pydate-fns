...
from .start_of_tomorrow import start_of_tomorrow

def test_returns_start_of_tomorrow():
    with patch('pydate.start_of_tomorrow.start_of_tomorrow.datetime') as mock_datetime:
        # Mock datetime.now() to return a specific date
        mock_datetime.now.return_value = datetime(2014, 9, 25, 14, 30, 45, 500)
        mock_datetime.side_effect = datetime
        
        result = start_of_tomorrow()
        assert result == datetime(2014, 9, 26, 0, 0, 0, 0)
...
def test_handles_dates_before_100_ad():
    with patch('pydate.start_of_tomorrow.start_of_tomorrow.datetime') as mock_datetime:
        ...
        result = start_of_tomorrow()
        assert result == datetime(14, 9, 26, 0, 0, 0, 0)
