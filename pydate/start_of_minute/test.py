...
import pytest
from .start_of_minute import start_of_minute

def test_returns_date_with_time_set_to_first_millisecond_of_minute():
    date = datetime(2014, 12, 1, 22, 15, 45, 400000)
    result = start_of_minute(date)
    assert result == datetime(2014, 12, 1, 22, 15, 0)
...
def test_returns_invalid_date_if_date_is_invalid():
    result = start_of_minute(float('nan'))
